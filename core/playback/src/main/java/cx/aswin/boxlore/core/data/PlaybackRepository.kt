...
class PlaybackRepository(
    ...
    fun generateAutoTranscript() {
        ...
        cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.trackAutoTranscriptRequested(episodeId, episode.podcastId, episode.audioUrl)
        if (isChaptersEmpty) {
            cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.trackAutoChaptersRequested(episodeId, episode.podcastId, episode.audioUrl)
        }
                ...
                override fun onMediaItemTransition(mediaItem: androidx.media3.common.MediaItem?, reason: Int) {
                    ...
                    repositoryScope.launch {
                        ...
                        val newPodcast: Podcast? = if (newEpisode.podcastId != null) {
                            ...
                            val database = cx.aswin.boxlore.core.data.database.BoxLoreDatabase.getDatabase(context)
    ...
    suspend fun playQueue(
        ...
        mediaController?.let { controller ->
            ...
            if (startPosMs > 0L) {
                cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setSeekSource("resume")
            }
    ...
    suspend fun toggleLike(episode: Episode, podcastId: String, podcastTitle: String, podcastImageUrl: String?) {
        ...
        if (existing != null) {
            ...
            val entity = cx.aswin.boxlore.core.data.database.ListeningHistoryEntity(
                episodeId = episode.id,
                podcastId = podcastId,
                episodeTitle = episode.title,
                episodeImageUrl = episode.imageUrl,
