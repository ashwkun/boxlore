...
import cx.aswin.boxlore.core.data.service.auto.AutoArtworkRepository
import cx.aswin.boxlore.core.data.service.auto.AutoBrowseContract
import cx.aswin.boxlore.core.data.service.auto.AutoMediaItemFactory
import cx.aswin.boxlore.core.data.service.auto.AutoPlayableSpec
...
class BoxLorePlaybackService : MediaLibraryService() {
    ...
    override fun onCreate() {
        ...
        val downloadCache = cx.aswin.boxlore.core.data.DownloadRepository.getDownloadCache(this)
        val streamCache = cx.aswin.boxlore.core.data.DownloadRepository.getStreamCache(this)
        ...
        player.addAnalyticsListener(object : androidx.media3.exoplayer.analytics.AnalyticsListener {
            ...
            override fun onPositionDiscontinuity(
                ...
                if (reason == Player.DISCONTINUITY_REASON_SEEK) {
                    ...
                    val source = cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.consumeSeekSource()
        ...
        player.addListener(object : Player.Listener {
            ...
            override fun onPlayWhenReadyChanged(playWhenReady: Boolean, reason: Int) {
                if (!playWhenReady) {
                    ...
                    cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setPauseReason(pauseReason)
            ...
            override fun onPlaybackSuppressionReasonChanged(reason: Int) {
                if (reason == Player.PLAYBACK_SUPPRESSION_REASON_TRANSIENT_AUDIO_FOCUS_LOSS) {
                    cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setPauseReason("audio_focus_loss_transient")
    ...
    private fun seekByConfiguredIncrement(player: ExoPlayer, deltaMs: Long, source: String) {
        ...
        cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setSeekSource(source)
    ...
    private fun resetLifecycleGuards(mediaItem: MediaItem?, initialPositionMs: Long) {
        ...
        cx.aswin.boxlore.core.data.PlaybackLifecycleSignals.effectiveSkipEndingMs = null
    ...
    private fun refreshActiveSkipConfiguration(
        ...
        serviceScope.launch {
            ...
            cx.aswin.boxlore.core.data.PlaybackLifecycleSignals.effectiveSkipEndingMs = 0L
    ...
    private fun resolveActiveIntroTarget(
        history: cx.aswin.boxlore.core.data.database.ListeningHistoryEntity?,
    ) {
    ...
    private fun refreshOutroBoundary(player: ExoPlayer, preferenceChanged: Boolean) {
        ...
        cx.aswin.boxlore.core.data.PlaybackLifecycleSignals.effectiveSkipEndingMs =
    ...
    private fun finishAtEffectiveEnd(player: ExoPlayer) {
        ...
        if (cx.aswin.boxlore.core.data.SleepTimerHolder.sleepAtEndOfEpisode) {
    ...
    private fun enforceEndOfEpisodeSleepAfterTransition(
        ...
        if (previousIndex >= 0) {
            ...
            cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setSeekSource("transition")
            player.seekTo(
                previousIndex,
    ...
    private fun prewarmAutoCollages() {
        serviceScope.launch {
            try {
                ...
                var mixtape = cx.aswin.boxlore.core.data.MixtapeEngine.build(
                    subscriptions = subscriptions.map { it.toAutoPodcast() },
                    history = history,
                    adaptiveRanking = cx.aswin.boxlore.core.data.MixtapeEngine.AdaptiveRanking(
                ...
                if (mixtape.episodes.size < 3) {
                    ...
                    mixtape = cx.aswin.boxlore.core.data.MixtapeEngine.build(
                        subscriptions = subscriptions.map { it.toAutoPodcast() },
                        history = history,
    ...
    private fun cx.aswin.boxlore.core.data.database.PodcastEntity.toAutoPodcast() =
    ...
    private fun startPlaybackSession(episodeId: String, currentItem: MediaItem?, fallbackEntryPoint: String? = null) {
        ...
        val pendingEntryPoint = cx.aswin.boxlore.core.data.analytics.PendingEntryPoint.consume()
    ...
    private fun endPlaybackSession(forceCompleted: Boolean = false, isTransition: Boolean = false) {
        ...
        if (playbackSessionStartTimeMs > 0 && currentEpisodeId != null) {
            ...
            if (
                ...
                val pauseReason = cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.consumePauseReason()
            ...
            serviceScope.launch {
                rankingFeedbackRepository.recordPlayback(
                    target = cx.aswin.boxlore.core.data.ranking.FeedbackTarget(
                        episodeId = currentEpisodeId,
            ...
            cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.flush()
    ...
    private suspend fun refillQueue(player: ExoPlayer) {
        ...
        val podcast = when {
            podcastEntity != null -> cx.aswin.boxlore.core.model.Podcast(
            ...
            isBriefing -> cx.aswin.boxlore.core.model.Podcast(
                id = "briefing_daily",
        ...
        val currentEpisodeItem = cx.aswin.boxlore.core.network.model.EpisodeItem(
        ...
        if (refilledEpisodeIds.isNotEmpty()) {
            ...
            cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.trackSmartQueueRefilled(
                cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.SmartQueueRefillEvent(
                    ...
                    usedServerRecommendations = cx.aswin.boxlore.core.data.SmartQueueEngine.SOURCE_PERSONALIZED_REC in sourceCounts ||
                        cx.aswin.boxlore.core.data.SmartQueueEngine.SOURCE_SERVER_REC in sourceCounts
                )
            )
        }
    }
    ...
    private inner class LibrarySessionCallback : MediaLibrarySession.Callback {
        ...
        override fun onCustomCommand(
            ...
            when (customCommand.customAction) {
                "SEEK_BACK" -> {
                    cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setSeekSource("seek_backward")
                    session.player.seekBack()
                    android.util.Log.d("AutoBrowse", "Seek backward")
                }
                "SEEK_FORWARD" -> {
                    cx.aswin.boxlore.core.data.analytics.AnalyticsHelper.setSeekSource("seek_forward")
        ...
        override fun onGetLibraryRoot(
            ...
            val rootItem = MediaItem.Builder()
                ...
                .setMediaMetadata(
                    MediaMetadata.Builder()
                        .setTitle(getString(cx.aswin.boxlore.core.data.R.string.auto_app_name))
                        .setIsPlayable(false)
                        .setIsBrowsable(true)
            ...
            return serviceScope.future {
                val item = if (
                    ...
                    (
                        ...
                        ?: AutoMediaItemFactory.browsable(
                            id = mediaId,
                            title = getString(cx.aswin.boxlore.core.data.R.string.auto_app_name),
                        )
                }
        ...
        private fun voiceEpisodeItem(
            ...
            groupTitle = getString(cx.aswin.boxlore.core.data.R.string.auto_group_search),
        )
        ...
        private fun voiceHistoryItem(
            ...
            groupTitle = getString(cx.aswin.boxlore.core.data.R.string.auto_group_continue),
        )
            ...
            return database.listeningHistoryDao()
                ...
                .map { history ->
                    AutoMediaItemFactory.fromHistory(
                        ...
                        groupTitle = getString(cx.aswin.boxlore.core.data.R.string.auto_group_liked),
                    )
                }
                .toMutableList()
        }
        ...
        private suspend fun handlePlayFromQueue(episodeId: String): MutableList<MediaItem> {
            ...
            return queue.drop(selectedIndex).map { episode ->
                AutoMediaItemFactory.fromEpisode(
                    ...
                    mediaIdPrefix = QUEUE_PREFIX,
                    groupTitle = getString(cx.aswin.boxlore.core.data.R.string.auto_group_queue),
                )
            }.toMutableList()
        }
        ...
        private fun getRootChildren(): List<MediaItem> {
            return listOf(
                AutoMediaItemFactory.browsable(
                    id = HOME_ID,
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_home),
                    subtitle = getString(cx.aswin.boxlore.core.data.R.string.auto_home_subtitle),
                ...
                AutoMediaItemFactory.browsable(
                    id = LIBRARY_ID,
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_library),
                    subtitle = getString(cx.aswin.boxlore.core.data.R.string.auto_library_subtitle),
                ...
                AutoMediaItemFactory.browsable(
                    id = DISCOVER_ID,
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_discover),
                    subtitle = getString(cx.aswin.boxlore.core.data.R.string.auto_discover_subtitle),
                ...
                AutoMediaItemFactory.browsable(
                    id = DOWNLOADS_ID,
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_downloads),
                    subtitle = getString(cx.aswin.boxlore.core.data.R.string.auto_downloads_subtitle),
        ...
        private suspend fun getHomeChildren(): List<MediaItem> {
            ...
            val newEpSubtitle = when {
                newEpCount == 0 -> getString(cx.aswin.boxlore.core.data.R.string.auto_new_none)
                newEpCount == 1 -> getString(cx.aswin.boxlore.core.data.R.string.auto_new_one)
                else -> getString(
            ...
            return listOf(
                ...
                AutoMediaItemFactory.browsable(
                    ...
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_drive_mix),
                    subtitle = getString(
                ...
                AutoMediaItemFactory.browsable(
                    ...
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_whats_new),
        ...
        private fun getDiscoverChildren(): List<MediaItem> {
            ...
            return listOf(
                AutoMediaItemFactory.browsable(
                    ...
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_drive_picks),
                    subtitle = getString(
                ...
                AutoMediaItemFactory.browsable(
                    ...
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_browse_genre),
                    subtitle = getString(
        ...
        private suspend fun getLibraryChildren(): List<MediaItem> =
            getSubscriptionsChildren() + listOf(
                AutoMediaItemFactory.browsable(
                    ...
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_liked_episodes),
                    mediaType = MediaMetadata.MEDIA_TYPE_FOLDER_PODCASTS,
                    childStyleExtras = AutoBrowseContract.listChildrenExtras(),
                    singleItemStyle =
                ...
                AutoMediaItemFactory.browsable(
                    ...
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_listening_history),
                    mediaType = MediaMetadata.MEDIA_TYPE_FOLDER_PODCASTS,
                    childStyleExtras = AutoBrowseContract.listChildrenExtras(),
                    singleItemStyle =
                        androidx.media3.session.MediaConstants.EXTRAS_VALUE_CONTENT_STYLE_LIST_ITEM,
        ...
        private suspend fun getHistoryChildren(): List<MediaItem> =
            database.listeningHistoryDao().getRecentHistoryList(50).map {
                AutoMediaItemFactory.fromHistory(
                    ...
                    groupTitle = getString(cx.aswin.boxlore.core.data.R.string.auto_group_history),
                )
            }
        ...
        private suspend fun getMixtapeChildren(): List<MediaItem> {
            ...
            val history = database.listeningHistoryDao().getRecentHistoryList(300)
            var result = cx.aswin.boxlore.core.data.MixtapeEngine.build(
            ...
            if (result.episodes.size < 3) {
                ...
                result = cx.aswin.boxlore.core.data.MixtapeEngine.build(
                    subscriptions = subscriptions,
                    history = history,
                    recommendations = recommendations,
                    adaptiveRanking = cx.aswin.boxlore.core.data.MixtapeEngine.AdaptiveRanking(
        ...
        private suspend fun getDrivePicksChildren(): List<MediaItem> {
            ...
            val items = episodes.map { (episode, podcast) ->
                AutoMediaItemFactory.fromEpisode(
                    ...
                    podcastTitle = podcast.title,
                    groupTitle = getString(cx.aswin.boxlore.core.data.R.string.auto_group_drive),
                )
            }
        ...
        private fun buildPodcastFolderItems(
            podcasts: List<cx.aswin.boxlore.core.model.Podcast>,
        ...
        private fun buildPlayAllItem(id: String, count: Int, source: String): MediaItem =
            AutoMediaItemFactory.playable(
                AutoPlayableSpec(
                    mediaId = id,
                    title = getString(cx.aswin.boxlore.core.data.R.string.auto_play_all, count),
