...
class PodcastRecommendationSeedsTest {
    ...
    fun progressRatioDrivesWeightBuckets() {
        val completed = TestFixtures.historyItem(
            episodeId = "1", isLiked = false, isCompleted = true,
        )
        val halfway = TestFixtures.historyItem(
            episodeId = "2", isLiked = false, isCompleted = false,
            durationMs = 100L, progressMs = 60L,
        )
        val barelyStarted = TestFixtures.historyItem(
            episodeId = "3", isLiked = false, isCompleted = false,
            durationMs = 100L, progressMs = 5L,
        )

        val seeds = buildRecommendationSeeds(
