...
import cx.aswin.boxlore.core.data.content.GroupedContentSections
import cx.aswin.boxlore.core.data.content.buildContentSignalProfile
import cx.aswin.boxlore.core.data.content.contentSectionsCacheKey
...
class PodcastRepository(
    ...
    ): cx.aswin.boxlore.core.data.content.ContentCatalogSnapshot? = withContext(Dispatchers.IO) {
        val now = System.currentTimeMillis()
    ...
    private fun readCachedContentCatalog(
        now: Long,
    ): cx.aswin.boxlore.core.data.content.ContentCatalogSnapshot? {
...
internal fun cx.aswin.boxlore.core.network.model.ContentCatalogResponse.toContentCatalogSnapshot(
        ...
        runCatching {
            cx.aswin.boxlore.core.data.content.ContentIntent(
                ...
                refreshPolicy = intent.refreshPolicy.toContentRefreshPolicy()
                    ?: cx.aswin.boxlore.core.data.content.ContentRefreshPolicy.SESSION,
                ...
                durationRange = intent.durationMinutes?.let {
                    cx.aswin.boxlore.core.data.content.ContentDurationRange(
                        minimumMinutes = it.min,
                        maximumMinutes = it.max,
                    )
                },
                diversity = cx.aswin.boxlore.core.data.content.ContentDiversityConstraints(
                    maximumItemsPerShow = intent.diversity.maxPerShow,
                    minimumDistinctShows = intent.diversity.minDistinctShows,
                ),
                quality = cx.aswin.boxlore.core.data.content.ContentQualityConstraints(
    ...
    return cx.aswin.boxlore.core.data.content.ContentCatalogSnapshot(
...
private fun String.toContentDaypart(): cx.aswin.boxlore.core.data.content.ContentDaypart? {
    return when (lowercase()) {
        "early_morning", "morning", "commute" ->
            cx.aswin.boxlore.core.data.content.ContentDaypart.MORNING
        "afternoon" -> cx.aswin.boxlore.core.data.content.ContentDaypart.AFTERNOON
        "evening" -> cx.aswin.boxlore.core.data.content.ContentDaypart.EVENING
        "late_night" -> cx.aswin.boxlore.core.data.content.ContentDaypart.LATE_NIGHT
        else -> null
    }
}

private fun String.toContentLayout(): cx.aswin.boxlore.core.data.content.ContentLayout? {
    return when (lowercase()) {
        "episode_rail" -> cx.aswin.boxlore.core.data.content.ContentLayout.EPISODE_RAIL
        "podcast_rail" -> cx.aswin.boxlore.core.data.content.ContentLayout.PODCAST_RAIL
        "compact_list" -> cx.aswin.boxlore.core.data.content.ContentLayout.COMPACT_LIST
        "protected_card" -> cx.aswin.boxlore.core.data.content.ContentLayout.PROTECTED_CARD
        else -> null
    }
}

private fun String?.toContentRefreshPolicy():
    cx.aswin.boxlore.core.data.content.ContentRefreshPolicy? {
    return when (this?.lowercase()) {
        "session" -> cx.aswin.boxlore.core.data.content.ContentRefreshPolicy.SESSION
        "manual" -> cx.aswin.boxlore.core.data.content.ContentRefreshPolicy.MANUAL
        "daypart" -> cx.aswin.boxlore.core.data.content.ContentRefreshPolicy.DAYPART
        "daily" -> cx.aswin.boxlore.core.data.content.ContentRefreshPolicy.DAILY
        else -> null
    }
}
