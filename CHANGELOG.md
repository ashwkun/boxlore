...
### Added
- Expressive listening history insights with session rollups, calendar and stats UI, Room persistence, and playback telemetry integration. ([#932](https://github.com/boxcreate/boxlore/pull/932)) <!-- impact:user-impact-high -->
- In‑memory session‑only SignalLog capturing each ranking engine mutation and model movement, with Debug UI panels (Signals, Taste, Model) replacing animated learner visualizations. ([#897](https://github.com/boxcreate/boxlore/pull/897))
### Changed
- Signal capture gating enabled by default in debug builds and disabled in release builds unless opted‑in; Learner engine now uses atomic exposure resolution with synchronized enable/disable of SignalLog. ([#897](https://github.com/boxcreate/boxlore/pull/897))
- Updated @libsql/client dependency to version 0.17.4 in build scripts. ([#894](https://github.com/boxcreate/boxlore/pull/894)) <!-- impact:no-user-impact -->
- Pinned transitive dependencies (Netty 4.1.136, BouncyCastle 1.84, jose4j 0.9.6, jdom2 2.0.6.1, commons‑compress 1.26.2, protobuf 3.25.5) and overridden protobufjs 7.6.5 and uuid 11.1.1 in scripts to resolve Dependabot Maven alerts and npm audit findings. ([#896](https://github.com/boxcreate/boxlore/pull/896)) <!-- impact:no-user-impact -->
- Modular Android hardening (plan A0–A8 / B0–B10): single AppContainer composition root with create/install for RSS and ranking; extracted core modules (:core:prefs, :core:downloads, :core:analytics, :core:ranking, :core:rss); MainActivity slimmed to shell over BoxLoreNavHost; DownloadServiceLauncher replaces reflective Class.forName; upgraded to AGP 9 toolchain and Develocity wiring; added module README contracts. ([#903](https://github.com/boxcreate/boxlore/pull/903)) <!-- impact:no-user-impact -->
- Removed .github/dependabot.yml and disabled Dependabot security updates via repository settings, stopping automatic version‑update and security PRs. ([#905](https://github.com/boxcreate/boxlore/pull/905)) <!-- impact:no-user-impact -->
- Feature and playback extraction under LOC caps: split home/playback, then defined info/library/explore/onboarding/analytics module boundaries so UI flows live under :feature:* and shared engines under :core:*. ([#919](https://github.com/boxcreate/boxlore/pull/919)) <!-- impact:no-user-impact -->
- Phase 2 package alignment: PostHog façade moved to :core:analytics; analytics glossary replaced; satellite/prefs migration; playback/database package aligned with Auto Phase C; catalog package aligned with Phase 2 final signoff. ([#921](https://github.com/boxcreate/boxlore/pull/921)) <!-- impact:no-user-impact -->
...
### Fixed
- Player UI state synchronization on app reopen fixed by preserving MediaController playback state; restoreLastSession() no longer forces isPlaying = false, keeping mini and full player in sync. ([#904](https://github.com/boxcreate/boxlore/pull/904)) <!-- impact:user-impact-medium -->
### Removed
- Firebase Hosting configuration migrated to aswin.cx; removed `.agents` Android‑ASO skill and related documentation. ([#932](https://github.com/boxcreate/boxlore/pull/932)) <!-- impact:user-impact-high -->
## [v0.0.10] - 2026-07-17
