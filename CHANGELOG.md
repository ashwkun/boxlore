...
### Added
- Added in‑memory session‑only SignalLog capturing each ranking engine mutation and model movement, with Debug UI panels (Signals, Taste, Model) replacing animated learner visualizations. ([#897](https://github.com/boxcreate/boxlore/pull/897))
### Changed
- Changed signal capture gating to be enabled by default in debug builds and disabled in release builds unless opted‑in; updated Learner engine to use atomic exposure resolution with synchronized enable/disable of SignalLog. ([#897](https://github.com/boxcreate/boxlore/pull/897))
- Changed @libsql/client dependency to version 0.17.4 in build scripts. ([#894](https://github.com/boxcreate/boxlore/pull/894)) <!-- impact:no-user-impact -->
- Changed architecture to use a modular AppContainer with create/install methods for RSS and ranking, added new core Gradle modules (:core:prefs, :core:downloads, :core:analytics, :core:ranking, :core:rss) and extensive test suite; refactored MainActivity into a lightweight shell delegating to BoxLoreNavHost, updated Gradle module ID policy, and removed reflective Class.forName usage in DownloadServiceLauncher. ([#903](https://github.com/boxcreate/boxlore/pull/903)) <!-- impact:no-user-impact -->
