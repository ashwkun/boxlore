...
- Refactored architecture to modular AppContainer with create/install methods for RSS and ranking, added core Gradle modules (:core:prefs, :core:downloads, :core:analytics, :core:ranking, :core:rss), extensive test suite, lightweight MainActivity shell delegating to BoxLoreNavHost, updated Gradle module ID policy, and removed reflective Class.forName usage in DownloadServiceLauncher. ([#903](https://github.com/boxcreate/boxlore/pull/903)) <!-- impact:no-user-impact -->
...
### Fixed
- Fixed player UI state synchronization on app reopen by preserving MediaController playback state; restoreLastSession() no longer forces isPlaying = false, keeping mini and full player in sync. ([#904](https://github.com/boxcreate/boxlore/pull/904)) <!-- impact:user-impact-medium -->
### Removed
- Firebase Hosting configuration for boxlore migrated to aswin.cx, and `.agents` Android‑ASO skill and related documentation cleaned up. ([#932](https://github.com/boxcreate/boxlore/pull/932)) <!-- impact:user-impact-high -->
### Security
